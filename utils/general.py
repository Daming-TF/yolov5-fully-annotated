...
def set_logging(name=None, verbose=VERBOSE):
    ...
    log = logging.getLogger(name)       # 创建记录器实例
    log.setLevel(level)     # 返回日志记录级别 level 的字符串表示
...
def check_suffix(file='yolov5s.pt', suffix=('.pt',), msg=''):
    ...
    if file and suffix:
        ...
        for f in file if isinstance(file, (list, tuple)) else [file]:
            # pathlib.suffix()获取文件后缀    .lower()返回小写字符串
...
def make_divisible(x, divisor):
    ...
    return math.ceil(x / divisor) * divisor     # 返回x的值上限- 不小于x的最小整数
